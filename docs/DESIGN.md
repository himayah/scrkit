...
> 背景11種のエフェクトを、差分ブロック/壁紙それぞれが互いに同期せず独立に無限巡回させる
> 拡張。§9.10でVortexSuctionへの強制収束を廃止して以降、両レイヤーとも単一の演出に
> 収束することはなく、周期的な暗転/復帰を挟みながら巡回を続ける)は
...
### 9.13 コンテンツマスクの残存フラッド: 4件の原因と修正 (v2.1.1、実機フィードバック複数ラウンド)

v2.1.0リリース後、SCRAPI/ScrViewer開発(別ブランチ)での実機検証中に、§9.12の対応後もなお
`content.source=desktop`相当の経路(実機の`/s`もマスク生成ロジックは共通)でマスクが広範囲に
フラッドする(壁紙が見えている領域まで前景と誤判定される)ケースが複数見つかった。いずれも
`src/core/ContentMask.{h,cpp}` / `WallpaperFit.cpp`側の原因で、テストで再現・修正しv2.1.1として
本ブランチに移植した。

1. **`RefineBoundaryMask`のカスケード的な誤検出(アイコン周辺のテクスチャノイズ)**: 境界セルを
   昇格しながら同じマスクを参照していたため、1つの昇格が隣接セルを新たに「境界セル」にし、
   細かいテクスチャの壁紙(写真の粒状ノイズ等)ではこれが連鎖してアイコン周辺の領域全体を
   巻き込んでいた。対応: 境界セルの判定はループ開始時点のマスクのスナップショットから固定し、
   ループ中の昇格では再判定しない。加えて、孤立した小さな昇格グループ(ノイズ由来)は
   最終的に切り捨てる。
2. **`SelectEvidencedRects`のエビデンス使い回し**: 矩形の証拠判定を「矩形内の全セル」で
   数えていたため、実際のウィンドウ群にまたがる大きな不可視オーバーレイ矩形が、内側にある
   本物のウィンドウの証拠を借りて採用され、壁紙全体をcontent化してしまうケースがあった
   (実機で99.6%のセルがcontent化)。対応: 証拠は「他のどの候補矩形にも属さない、その矩形
   専用のセル」だけで数える。証拠が0の矩形は棄却でなく保留とし、棄却された矩形を除いて
   再集計する(内側の本物ウィンドウは、外側の偽オーバーレイが落ちた後に採用され得る)。
3. **壁紙リファレンスの生成精度**: (a) ニアレストネイバー法での拡大縮小がテクスチャの細かい
   壁紙でエイリアシングノイズを生み(`ResampleRgbaSmooth`へ変更: ダウンスケールは面積平均、
   アップスケールは線形)、(b) `Fill`/`Span`時のクロップ位置探索が4px刻みで粗かった(1px単位の
   再探索を追加)、(c) 差分前のボックスぼかし半径が固定5x5だったため、リファレンスが微細スケール
   でずれる壁紙では常にノイズが残っていた(`ChooseBlurRadius`: 静かなタイルでの閾値超過率が
   十分下がるまで半径2→3→4→6→8→12と段階的に広げる、データ駆動のアダプティブ選択に変更)。
4. **明るさ補正ゲインの多数派前提の破綻(v2.1.1で新規に発見)**: `ComputeRobustBrightnessGain`は
   「壁紙(背景)ピクセルが画面の多数派である」という前提で、キャプチャ/壁紙の輝度比の中央値を
   補正ゲインとして採用していた。実機でターミナル+ブラウザ2枚が画面の約9割を覆う状況では
   この前提が崩れ、中央値がウィンドウ側のピクセル比(実測1.28倍)に乗ってしまい、そのゲインを
   壁紙全体に適用した結果、正しく一致していたはずの背景ピクセルまで「異なる」と誤判定される
   割合が17%→57%に悪化していた(壁紙リファレンス自体は正常だった)。対応:
   `ComputeContentMask`/`ChooseBlurRadius`/`RefineBoundaryMask`に`gainExcludeRects`(OS報告の
   ウィンドウ矩形、証拠選別前の生の候補一覧)を追加し、ゲインの比のサンプルからそれらの矩形内の
   ピクセルを除外する。`AppController::Initialize`はウィンドウ矩形の取得を壁紙合成より前段に
   引き上げ、ゲイン計算と後段の矩形証拠付けの両方で同じ一覧を共有する。

いずれも合成データでの回帰テストを追加し(`ContentMask_GainExcludeRectsRecoverBackgroundOnAMostlyWindowedDesktop`
ほか)、Linux上のコアテストは全緑。原因の切り分けは実機で保存した`debug_capture.bmp`/
...
- `src/core/effects/`配下のエフェクトクラス名(`SuctionEffect`/`VortexSuction`等)、内部
  ...
  (「過去ログは書き換えない」方針)に従い、文中の旧名称もそのまま残している。

### 9.15 §9.14でのSaverIdentity命名の修正、旧版の特徴に基づく記述の全体点検 (v3.0.1、ユーザー指摘)

§9.14でSCRAPIの`SaverIdentity`(`m.saver`)を`{"jp.himayah.scrkit.spiral-suction", "Spiral Suction", version}`
としたが、ユーザーから「README.mdの`Spiral Suction`という記述が旧版(エフェクトシステム拡張前、
背景が強制的にらせん吸い込みへ収束していた頃)の特徴に基づいている」との指摘を受けた。
指摘の通り、VortexSuctionは現在は上物14種のうちの1エフェクトに過ぎず(§9.10で背景の強制収束は
既に廃止済み)、この`.scr`を「Spiral Suction」という個別ブランドで呼ぶこと自体が、プロジェクト名
リネームの動機(実態と乖離した名称)と同種の問題を、今度はセーバー単位で再導入していた。

- `SaverIdentity`を`{"jp.himayah.scrkit", "ScrKit", version}`に変更(プロジェクト名と統一)。
  本プロジェクトの`.scr`は現状ScrKit.scr一つのみであり、独自のエフェクト別ブランド名を
  別途持たせること自体が将来また陳腐化するリスクを生むため、YAGNIに倣い廃止した(将来
  ScrKit配下に複数の`.scr`が並ぶ段になって初めて、区別用の識別子を検討すれば良い)。
  ScrViewerのステータス表示(`ViewerMain.cpp`)は`saver().name`をそのまま表示するため、
  接続時に「Connected: ScrKit ...」と表示されるようになる。
- README.md冒頭のコンポーネント一覧、`docs/DESIGN.md`のタイトル、`docs/DESIGN_VIEWER.md`/
  `docs/SCRAPI_SPEC.md`中の「spiral-suction (ScrKit.scr)」という言い回しから、個別ブランド名
  としての「Spiral Suction」表記を除去し、単に`ScrKit.scr`と呼ぶよう統一。
- 同じ観点で他の記述も再点検し、本書冒頭(1行目直下)の拡張についての説明が「上物・背景の
  エフェクトが**らせん吸い込みの前に**演出する」という、背景の強制収束が現行仕様だった
  頃の説明のまま残っていたのを発見・修正(現行仕様は§9.10の通り、両レイヤーとも単一の
  演出に収束せず無限巡回する)。
- `docs/DESIGN_EFFECTS.md`・`docs/REVIEW_DESIGN_EFFECTS.md`・`要件.txt`・`追加要件.txt`
  (凍結された過去ログ)、および本書内の§9.x履歴各節(当時の記述をそのまま残す方針)は、
