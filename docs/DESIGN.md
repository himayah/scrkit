...
### 9.8 残作業 (未着手、次回再開時のメモ)

- **壁紙合成の明るさが実際の画面キャプチャと大きくズレている**: 実機ログで、
  `desktopCapture`の平均輝度(サンプリング値、148〜197/255)に対し、
  `core::CompositeWallpaper`で合成した`compositedWallpaper`の平均輝度が96/255と大きく
  暗い側にズレている事例を確認した。この結果、本来「差分なし」であるはずの画面の
  大部分(実測で画面の97%程度)が誤って`core::ContentMask`に「差分ブロック」と判定されて
  しまっている。9.2で対応した壁紙表示スタイル(`GetSystemWallpaperFitMode`)の判定自体が
  誤っている(レジストリ値の読み取り・マッピングミス)か、`CompositeWallpaper`の合成
  ロジック自体に別のバグがあるか、あるいは壁紙ファイルのデコード結果自体が実際の表示と
  色味が異なる(色空間/ガンマ等)可能性がある。次回はまずこの3つを切り分けて調査する。
  一時的に`AppController::Initialize`へ明るさ比較ログを再度仕込んで実機検証するのが早い
  (9.7で追加・削除した`PerfTimer`と同様の手法)。
- **README.mdが現行アーキテクチャと乖離している**: `README.md`は「アイコン/ウィンドウを
  `EnumWindows`等で個別に実位置取得し矩形として描画する」という、`04e8ad7`(差分方式への
  全面置き換え)より前の旧アーキテクチャの説明のまま更新されていない
  (ソフト概要・技術的特徴・使用上の注意点の各節)。`docs/DESIGN.md`は都度更新済みだが、
  `README.md`は今回のGitHub Actionsビルド手順追記のみを行い、内容の全面見直しは
  未着手。次回、現行の差分ベース方式(`core::ContentMask`/`core::WallpaperFit`)に
  合わせて書き直す必要がある。
