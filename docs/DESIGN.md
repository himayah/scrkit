...
### 9.7 起動時のブラックアウト解消 (追加要望)

`/s`起動直後、実画面キャプチャ後に生成する全画面ウィンドウ(`WS_POPUP`+`WS_EX_TOPMOST`)は
従来`WS_VISIBLE`付きで作成しており、`AppController::Initialize`(壁紙デコード・
`ContentMask`の差分計算・テクスチャ生成などCPU処理を含む)が完了して最初の`Draw()`が
呼ばれるまでの間、何も描画されていない黒いウィンドウが画面全体を覆ってしまっていた
(=起動時に一瞬ブラックアウトして見える)。

対応として、ウィンドウは`WS_VISIBLE`を付けずに**非表示のまま**生成し、実際のデスクトップは
そのまま表示され続けるようにした。`SaverWindow::RunMessageLoop`はOpenGL初期化・
`AppController::Initialize`完了後、メインループに入る**前**に一度だけ`Update`/`Draw`/
`SwapBuffers`を実行して最初の1フレームをバックバッファに描画し、その直後にコールバック
(`onReadyToShow`)を呼んでからウィンドウを`ShowWindow`で表示する。ウィンドウが実際に
見えるようになった瞬間には既に(実キャプチャ由来の)完成した最初のフレームが表示される
ため、黒画面や未初期化状態が一切見えない。カーソル非表示・フォアグラウンド化もこの
タイミングまで遅延させている。OpenGL初期化に失敗した場合は、この経路を通らないため
明示的に`ShowWindow`してからエラーダイアログを表示する(従来通りの見た目を維持)。
プレビュー(`/p`)は元々子ウィンドウとして`WS_VISIBLE`のまま作成しており、この対応の
対象外(`onReadyToShow`省略時は何もしない)。
