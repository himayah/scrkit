...
### 9.7 起動時のブラックアウト解消 (追加要望、実機フィードバックで方式変更)

`/s`起動直後、実画面キャプチャ後に生成する全画面ウィンドウ(`WS_POPUP`+`WS_EX_TOPMOST`)は
`WS_VISIBLE`付きで作成しているため、`AppController::Initialize`(壁紙デコード・
`ContentMask`の差分計算・テクスチャ生成などCPU処理を含み、実機で2秒程度かかることがある)
が完了して最初の`Draw()`が呼ばれるまでの間、何も描画されていない黒いウィンドウが
画面全体を覆ってしまっていた(=起動時にブラックアウトして見える)。

最初の対応として「ウィンドウを`WS_VISIBLE`なしで非表示のまま生成し、初期化完了後に
`ShowWindow`で表示する」方式を試みたが、実機では改善しなかった(2秒程度の黒画面が
残った)。DWM(デスクトップウィンドウマネージャ)が全画面トップモストウィンドウを
`ShowWindow`で初めて表示する際の合成タイミングに依存していた可能性がある。

そこで方式を変更し、**ウィンドウは`WS_VISIBLE`付きで即座に表示したまま**、
`SaverWindow::RunMessageLoop`の先頭で「実画面キャプチャ画像をそのままテクスチャ化して
全画面に1枚描画し`SwapBuffers`する」処理を`AppController::Initialize`より**前**に
追加した。これは実際のデスクトップのスナップショットなので、ウィンドウがデスクトップを
覆った瞬間に見た目がデスクトップとほぼ区別つかない絵に置き換わるだけになる。そのあとで
時間のかかる`AppController::Initialize`(同じキャプチャ画像を実際のcontentフェーズの
テクスチャとしても再利用する)を実行し、完了すればメインループの最初のフレームが
このスナップショットから自然に差分ブロックが浮かび上がる形で始まる。`ShowWindow`の
タイミングに依存しないため、DWMの合成挙動に左右されず確実に黒画面を回避できる。
プレビュー(`/p`)は`desktopCapture`を渡さないため、このスナップショット表示はされず
(元々の縮小キャンバス動作のまま)、この対応の影響を受けない。
