...
独立したレイヤーとして専用の多彩な演出エフェクト(上物14種・背景11種)で変形・変色させながら
消していく Windows スクリーンセーバー (`.scr`) です。上物は、渦を巻きながら**らせん軌道で
吸い込まれて消える**演出(VortexSuction)を含む6種類の消滅演出のいずれかで消え、背景(壁紙)は
最終的に必ずこの**らせん軌道での吸い込み**演出(BackgroundSuction)で、画面上を漂う一点へ
吸い込まれて消えます。すべて吸い込まれると画面が黒くなり、元の壁紙へフェードインして最初の
状態に戻る（無限ループ）動作をします。エフェクトシステムを無効化する
(`Effects.Enabled=0`)と、上物・背景ともに常にこのらせん吸い込みだけで消える、拡張前(v1)と
同じシンプルな見た目に戻せます。詳細は
...
> キャプチャ画像のクリッピングとしてエフェクト演出・吸い込みアニメーションの対象になります。**実際のアイコン・
> ウィンドウを移動・削除したり、OS の設定を変更したりすることは一切ありません** -- 画面の
> 内容を「読む」だけで、何も動かしません。画面キャプチャに失敗した場合(別シェル利用時、
> セキュリティソフトによるブロックなど)は、吸い込み対象を検出できないだけで、背景画像
> フェーズ以降は通常どおり動作します。
...
- 実際の画面キャプチャと壁紙画像を比較して検出した「差分ブロック」(アイコン・タスクバー・
  開いているウィンドウなど、壁紙の上に何か描かれている箇所) → 背景画像 (NxN 粒子に分割)
  の順に、それぞれ専用のエフェクトで変形・消滅していきます。差分ブロックは6種類の消滅演出
  (渦を巻きながら吸い込まれる VortexSuction を含む)のいずれかで、背景画像は必ずらせん軌道
  で中心へ吸い込まれる演出 (BackgroundSuction。中心に近づくほど角速度が上がり、らせん状に
  歪んでいく) で消滅します。差分のない箇所は最初から壁紙がそのまま見えているため、結果的に
  「変化のない部分は透明化される」演出になります。
- 全て吸い込まれると画面は黒一色になり、その後、元の壁紙へフェードイン (alpha 0→1) し、
  差分ブロックを元の位置に再表示してから、また最初のステップへ戻ります。
...
- フルスクリーン実行時、差分ブロックは実際に画面キャプチャで検出した位置・見た目
  (クリッピング画像) で描画されます。画面キャプチャに失敗した場合は差分ブロックが
  0件になり、このフェーズは実質スキップされて背景画像フェーズから始まります。プレビュー
  表示時 (`/p`) は画面キャプチャを行わない設計のため、常に背景画像フェーズから始まって
  見えます。
