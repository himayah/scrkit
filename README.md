...
- **`ScrKit.scr`** -- 本リポジトリに同梱されている、実際に動作するスクリーンセーバー本体
  (詳細は下記)。
...
以下、同梱のスクリーンセーバー本体 `ScrKit.scr` について説明します。
...
> (アイコン・タスクバー・開いているウィンドウなど)」だけを検出します(ウィンドウの内部が
> 壁紙と同じ色で差分が出ない場合に備え、開いているウィンドウの位置・大きさも読み取り専用で
> 補助的に参照します)。検出した箇所だけが
...
- **検出の頑健性向上 (実機フィードバックに基づく改善)**: 上記の基本比較だけでは拾えなかった
  実機特有の失敗モードを、ユーザーからの実機レポートを元に切り分けて個別に対応済みです。
  ...
  加えて、白いウィンドウが壁紙の白い領域と重なると、ウィンドウの地が壁紙と完全に一致して
  差分が一切出ず、上物の長方形の内部が透過に侵食されてしまう問題 → `EnumWindows` と
  `DwmGetWindowAttribute` で取得した実際の可視ウィンドウ/タスクバーの矩形
  (`platform::EnumerateVisibleWindowRects`)を、ピクセル差分が裏付けたものに限って
  (`core::SelectEvidencedRects`)採用し、その内部を必ず不透過にします
  (`core::ForceRectsIntoMask`、矩形の縁は `RefineBoundaryMask` がピクセル単位で追従)。
  ウィンドウの位置を読み取るだけで、移動・操作は一切行いません。
...
- 本ソフトウェアは実際の画面の内容をフルスクリーン実行時に**読み取り専用**でキャプチャし、
  ...
  (壁紙との差分がウィンドウ内で出ない場合の補完のため、開いているウィンドウの位置と大きさも
  読み取りますが、これも読み取りのみです。)
...
テストが `tests/` にあります（外部依存ゼロの自作テストハーネス使用、現在 234 ケース）。
